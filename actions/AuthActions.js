...
export const signupUser = ({ email, password, phone, firstname, lastname  }) => {
  return async (dispatch) => {
    ...
    try {
      let user = await firebase.auth().createUserWithEmailAndPassword(email, password).then(function(data){
      console.log('uid',data.user.uid)
      firebase.database().ref(`/users/${data.user.uid}/userDetails`).set({
        email,
        phone,
        firstname,
        lastname,
      ...
      console.log('database has been put up')
      console.log(user);
      loginUserSuccess(dispatch, user);
      dispatch({
        type: ERROR_SET,
